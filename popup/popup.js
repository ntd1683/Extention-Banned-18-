...
document.addEventListener('DOMContentLoaded', function() {
  var searchCount = 0;
    chrome.storage.local.get({ searchHistory: [] }, function(result) {
      searchCount = result.searchHistory.length;
      var searchCountElement = document.getElementById('searchCount');
      searchCountElement.textContent = searchCount;
    });
  
    var manageSearchButton = document.getElementById('manageSearchButton');
    manageSearchButton.addEventListener('click', function() {
        chrome.tabs.create({ url: '../manage/manage.html'});
    });

    var deleteData = document.getElementById('deleteData');
    deleteData.addEventListener('click', function() {
      $check = prompt('Are you sure you want to delete', 123);
